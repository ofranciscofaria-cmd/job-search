...
- Canva cover letter: never edit the reference design. `copy-design` of `DAHVX9hPx2g` ("Cover Letter - Francisco Faria x LIZY", 1 page) with `page_numbers: [1]`, then `read-design` with `open_transaction: true` and edit (element IDs are the same in every copy; locator = `<pageId>-<elementId>`):
  - `LB7l029kCKngLnhn` letter body (replace the whole text: paragraphs, then "Looking forward to hearing from you." or similar close, then "Best regards,"). Keep it to about 3 short paragraphs so it ends above the signature image `LB5py8pM30ybk519` (top about 840); if the body box bottom (top + height) goes past about 835, shorten the text.
  - `LBfkjvFBvq9Lcy8v` greeting ("Dear <Company> Team," or the hiring manager's name if the posting gives it).
  - `LBn1yF3xQlps3dwh` date, format "25th of September of 2026" (the day it is written).
  - `LBfQ13qCmjPtDwT9` address and phone ("Keizer Karelstraat, Gent, Belgium" + Belgian number; Portuguese number instead only for Portugal-based roles). Leave the name, email/LinkedIn and decorative shapes alone.
  - Commit (`edit-design` with `finalize: "commit"`), rename "Cover Letter - Francisco Faria x <Company>", export PDF (A4), check the thumbnail for overflow.
- Structure: (1) why this company and role, using motivations he has confirmed, ask if unknown, never fake passion; (2) proof from his real work with numbers; (3) address the biggest gap honestly in one sentence, only if it is a real gap; (4) short close. Match the posting's tone and language. No em dashes.
...
- Then set Status CV ready, CV (Canva) edit link, attach the PDF to CV PDF (Canva export URLs expire: upload via Notion attachment from the export URL, filename `CV - Francisco Faria x <Company>.pdf`, see Tailoring step 8), Last update. Cover letter: set the `Cover letter (Canva)` column to the edit link and attach the PDF to the `Cover letter PDF` column (uploaded via Notion attachment, filename `Cover Letter - Francisco Faria x <Company>.pdf`; the same file_upload id can also be embedded in the body). Also put the Canva link and PDF at the top of "## Texts" in the page body, followed by the letter text ready to paste. Notify via ntfy with the Notion row link.
