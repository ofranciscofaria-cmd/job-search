...
- Then set Status CV ready, CV (Canva) edit link, attach the PDF to CV PDF (Canva export URLs expire: upload via Notion attachment from the export URL, filename `CV - Francisco Faria x <Company>.pdf`, see Tailoring step 8), Last update. Cover letter: put the Canva edit link and the PDF (uploaded via Notion attachment, filename `Cover Letter - Francisco Faria x <Company>.pdf`) at the top of "## Texts" in the page body, followed by the letter text ready to paste. Notify via ntfy with the Notion row link.
