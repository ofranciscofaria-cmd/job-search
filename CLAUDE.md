...
- Then set Status CV ready, CV (Canva) edit link, attach the PDF to CV PDF (Canva export URLs expire: upload via Notion attachment from the export URL, filename `CV - Francisco Faria x <Company>.pdf`, see Tailoring step 8), Last update. Notify via ntfy with the Notion row link.
